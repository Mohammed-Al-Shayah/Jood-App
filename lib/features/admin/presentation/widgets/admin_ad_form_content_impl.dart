...
part 'admin_ad_form_support.dart';
...
class _AdminAdFormContentState extends State<AdminAdFormContent> {
  ...
  Widget _scheduleField({
    ...
    return TextFormField(
      ...
      decoration: adminInputDecoration(label).copyWith(suffixIcon: Icon(icon)),
      validator: validator,
    );
  }
  ...
  Future<void> _save() async {
    ...
    if (scheduleEnd.isBefore(scheduleStart)) {
      setState(() {
        _loadError =
            'Ad end date and time must be after the start date and time.';
      });
      return;
    }
